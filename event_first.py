        ...
        if p_short >= 0:
            ...
            if age > cfg.reentry_window or out.at[i, "segment_id"] != out.at[p_short, "segment_id"]:
                ...
                if reenter:
                    ...
                    csum = int(c1) + int(c2) + int(c3) + int(c4)
                    if csum >= cfg.min_confirmations:
                        signal[i] = -1
                        side[i] = "short"
                        confirm_count[i] = csum
                        confidence[i] = min(1.0, (csum / 4.0) * out.at[i, "box_confidence"])
                    p_short = -1
        ...
        if p_long >= 0:
            ...
            if age > cfg.reentry_window or out.at[i, "segment_id"] != out.at[p_long, "segment_id"]:
                ...
                if reenter:
                    ...
                    c4 = bool(out.at[i, "close"] >= out.at[i, "box_midline"] - cfg.recapture_to_mid_frac * width)
                    csum = int(c1) + int(c2) + int(c3) + int(c4)
                    if csum >= cfg.min_confirmations and signal[i] == 0:
                        signal[i] = 1
                        side[i] = "long"
                        confirm_count[i] = csum
                        confidence[i] = min(1.0, (csum / 4.0) * out.at[i, "box_confidence"])
